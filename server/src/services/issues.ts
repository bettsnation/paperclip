...
function buildAdvanceAction(
  ...
  const action: Omit<PipelineAction, "pipelineRunId"> = {
    ...
    runPatch: { currentStageId: nextStage.id, stateJson: newStateJson, updatedAt: new Date() },
  };
  ...
  switch (currentStage.onReject) {
    case "previous": {
      if (currentGroupIdx <= 0) {
        return {
          ...
          runPatch: { status: "failed", stateJson: clearedStateJson, updatedAt: new Date() },
        };
      }
      ...
      return {
        ...
        runPatch: { currentStageId: prevStage.id, stateJson: clearedStateJson, updatedAt: new Date() },
      };
    }
    ...
    case "restart": {
      ...
      return {
        ...
        runPatch: { currentStageId: firstStage.id, stateJson: clearedStateJson, updatedAt: new Date() },
      };
    }
